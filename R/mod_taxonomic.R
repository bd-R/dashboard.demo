...
mod_taxonomic_server <-
  function(input, output, session, data_taxo) {
    ...
    observeEvent(event_data("plotly_click", source = "taxobar_1"), {
      new <- event_data("plotly_click", source = "taxobar_1")
      selected_bar_1(new)
    })
    ...
    output$back_1 <- renderUI({
      if (length(selected_bar_1()))
        actionButton(ns("clear_bar_1"), "Back/Reset", icon("chevron-left"))
    })
    ...
    observeEvent(event_data("plotly_click", source = "taxobar_2"), {
      new <- event_data("plotly_click", source = "taxobar_2")
      selected_bar_2(new)
    })
    ...
    output$back_2 <- renderUI({
      if (length(selected_bar_2()))
        actionButton(ns("clear_bar_2"), "Back/Reset", icon("chevron-left"))
    })
